...
export default function CareerStats({ profile }: { profile: Profile }) {
  ...
  const goldStakeWins = Object.values(profile.deck_usage)
    .map((deck) => deck.wins?.['8'] ?? 0)
    .reduce((sum, x) => sum + x, 0)
  const goldStakeLosses = Object.values(profile.deck_usage)
    .map((deck) => deck.losses?.['8'] ?? 0)
    .reduce((sum, x) => sum + x, 0)
  const goldStakeWinPercentage =
    Math.round(
      (goldStakeWins / (goldStakeWins + goldStakeLosses || 1)) * 10000,
    ) / 100
  ...
  stats['total_tarots_used'].value = Object.entries(
    profile.consumeable_usage,
  ...
  stats['total_planets_used'].value = Object.entries(
    profile.consumeable_usage,
  ).reduce((sum, [key, { count }]) => {
  ...
  stats['total_spectrals_used'].value = Object.entries(
    profile.consumeable_usage,
  ).reduce((sum, [key, { count }]) => {
  ...
  stats['total_consumables_used'].value = Object.values(
    profile.consumeable_usage,
  ).reduce((sum, { count }) => {
    return sum + count
  }, 0)
  ...
  const gamesPlayed =
    profile.career_stats['c_wins'] + profile.career_stats['c_losses'] || 1
  const winPercentage =
    Math.round((profile.career_stats['c_wins'] / gamesPlayed) * 10000) / 100
  ...
  return (
    <div className={styles.careerStats}>
      {Object.entries(sections).map(([section, keys]) => {
        return (
          <div key={section}>
            <div className={styles.careerStatsLabel}>{section}</div>
            ...
            {keys.map((key) => {
              ...
              return (
                <Stat
                  ...
                  value={value}
                  extra={
                    settings.showPerGameStats.enabled && stats[key].perGame
                      ? Math.round((stats[key].value / gamesPlayed) * 10) / 10
                      : undefined
                  }
